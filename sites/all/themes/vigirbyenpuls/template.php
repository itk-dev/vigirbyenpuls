...
function vigirbyenpuls_preprocess_page(& $variables, $hook) {
	...
	// $variables['css_alt'] = vigirbyenpuls_css_reorder($variables['css']);
	// $variables['styles'] = drupal_get_css($variables['css_alt']);

	//	
	// Add taxonomy-term to body class. 
	// Also for selected uris generated additional template suggestions.
	//
	...
	if (module_exists('path')) {
        ...
        { // Node field header exists
            ...
            { // And is liked to a valid node
				...
				$variables[header_graphic]= '<div id="header-graphic">' .
											' <img src="/' . $node->field_graphic[0][filepath] . '">' .
											' <div class="caption"><span class="span">' . $node->title . '</span></div>' .
											'</div>';
			}
		}
		*/
		...
		if ($last_part != "edit") {
			...
			if (arg(0) == "node" && is_numeric(arg(1))) {
				...
				// Check if node assigned category, and retrive category if so
